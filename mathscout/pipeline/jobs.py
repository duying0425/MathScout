...
class CrawlJobRunner:
    ...
    max_wait_for_scheduled_task_seconds = 30
    ...
    def create_job(self, name: str, urls: list[str], discover_links: bool = False) -> dict[str, str | int]:
        job = CrawlJob(
            ...
            source_filter={"urls": urls, "discover_links": discover_links},
        )
    ...
    def run_job(self, job_id: str) -> dict[str, str | int]:
        ...
        deep_mode = bool(source_filter.get("discover_links", False))
        # 路径前缀：只跟踪起始 URL 所在目录下的链接
        seed_urls: list[str] = source_filter.get("urls") or []
        path_prefix = _common_path_prefix(seed_urls)
        processed = 0
        while True:
            ...
            runnable = or_(
                ...
                (CrawlTask.status == CrawlStatus.failed) & (CrawlTask.retries < self.max_retries),
            )
            ...
            if task is None:
                scheduled_task = self.session.scalar(
                    ...
                    .where(
                        CrawlTask.job_id == parsed_job_id,
                        runnable,
                        CrawlTask.not_before.is_not(None),
                        CrawlTask.not_before > now,
                    )
                    .order_by(CrawlTask.not_before.asc())
                )
                if scheduled_task is not None:
                    wait_seconds = min(
                        (scheduled_task.not_before - now).total_seconds(),
                        self.max_wait_for_scheduled_task_seconds,
                    )
                    if wait_seconds > 0:
            ...
            try:
                if task.task_type == "discover_links":
                    ...
                    result = CrawlPipeline(self.session).fetch_and_store(task.url)
                    task.status = (
                        CrawlStatus.blocked
                        if result.get("pipeline_status") == "login_required"
                        else CrawlStatus.succeeded
                    )
                    if deep_mode and result.get("pipeline_status") == "crawled":
                        self._add_discovered_links(job, task, result, path_prefix)
                ...
                if task.retries < self.max_retries:
                    backoff = self.retry_backoff_base_seconds * (2 ** (task.retries - 1))
                    task.not_before = datetime.utcnow() + timedelta(seconds=backoff)
            finally:
                task.updated_at = datetime.utcnow()
                self.session.commit()
    ...
    def _add_discovered_links(
        self, job: CrawlJob, source_task: CrawlTask, fetch_result: dict,
        path_prefix: str = "",
    ) -> None:
        """从刚抓取的页面里提取同域、同路径前缀的链接，加进任务队列。"""
        raw_path = fetch_result.get("raw_path")
        content_type = fetch_result.get("content_type", "")
        base_url = fetch_result.get("url") or source_task.url
        if not raw_path or "html" not in content_type:
            return

        allowed_domain = urlparse(base_url).netloc
        try:
            from scrapling.parser import Adaptor
            html = Path(raw_path).read_bytes().decode("utf-8", errors="ignore")
            page = Adaptor(html, url=base_url)
            hrefs = {a.attrib.get("href", "") for a in page.css("a")}
        except Exception:
            return
        ...
        existing_urls: set[str] = {
            ...
            for (url,) in self.session.execute(
                select(CrawlTask.url).where(CrawlTask.job_id == job.id)
            ).all()
        }
...
def _common_path_prefix(urls: list[str]) -> str:
    """从种子 URL 列表中提取公共路径前缀（到最后一个 '/' 为止）。

    例如 ['https://example.com/a/b/c.html', 'https://example.com/a/b/d.html']
    → '/a/b/'，只跟踪该目录下的链接。
    """
    if not urls:
        return ""
    paths = [urlparse(u).path for u in urls if u.startswith("http")]
    if not paths:
        return ""
    # 找公共前缀，截断到最后一个 /
    prefix = paths[0]
